...
import { document, savedExercise, semester } from "./database/schema";
  ...
  try {
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        if (!subscriptionId) break; // Einmalzahlung — kein Abo zu pflegen.
  ...
  const results = await Promise.all(
    texts.map(async (item) => {
      ...
      try {
        ...
        return { id: item.id, content: text.trim() || item.content };
      } catch {
