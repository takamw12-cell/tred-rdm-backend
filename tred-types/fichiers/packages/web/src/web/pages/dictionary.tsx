  ...
  const filtered = useMemo(() => {
    ...
    return terms.filter((term) => {
      ...
      const matchQ =
        ...
        pick(term.translation, locale).toLowerCase().includes(q);
              ...
              >
                <Card className="h-full">
                  <CardContent className="flex h-full flex-col pt-0">
                    ...
                    <p className="reading-scalable text-sm leading-relaxed">{pick(term.definition, locale)}</p>
                    <div className="text-muted-foreground bg-secondary/50 mt-3 flex gap-2 rounded-lg p-2.5 text-xs italic">
                      ...
                      <span>{pick(term.example, locale)}</span>
                    </div>
